...
class pvaClient:
    def __init__(self, nth=1):
        ...
        self.frame_id = None
        self.trt_engine_path = 'auto_PtychoNN_ds_40.trt'
    ...
    def frame_process(self, ):
        while self.thr_exit == 0:
            ...
            rows  = dims[0]['size']
            cols  = dims[1]['size']
            frame = pv['value'][0]['intValue'].reshape((rows, cols))
            ...
            if (len(in_mb)==self.max_batch_size):
                ...
                trt_outputs = np.asarray(trt_outputs1[1])      
                print(trt_outputs.shape)
                print("Execution Times ", times)
            #for _ in in_id:
            self.server.update(self.channel_name, self.frame_producer(frm_id, trt_outputs1[1]))
